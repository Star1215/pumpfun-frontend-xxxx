...
export default function MyProfile() {
  ...
  useEffect(() => {
    const initialize = async () => {
      ...
      try {
        ...
        toast.dismiss(id);
        toast.failure("Failed to initialized program: " + err.message);
      }
    };
  ...
  const handleDashboardSet = async () => {
    if (ownerAddress === '' || feeRecipient === '' || tradingFee === '' || tradingFee === '0') {
  ...
  return (
    <section className={`z-10 flex flex-col items-center gap-4 mx-auto pt-[80px] px-4`}>
      <div className='flex flex-col gap-4'>
        <div className="z-10 flex gap-6 items-center">
          ...
          <div className='flex flex-col'>
            <p className='text-sm sm:text-lg text-white font-bold'>@{profileData?.username}</p>
            <div className='flex flex-col gap-1'>
              <p className='text-md text-white'>{profileData?.followers} followers</p>
              ...
              <div className='flex items-center gap-2'>
                <div className='flex items-center'>
                  <p className='text-sm text-[#FF3131]'>Likes received: {profileData?.likes}</p>
                ...
                <div className='flex items-center'>
                  <p className='text-sm text-[#97FF73]'>Mentions received: {profileData?.mentions}</p>
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className='flex flex-col gap-1 mb-8'>
          <input type="text" className='w-full bg-transparent text-[13px] text-white border border-[#808080] p-2 rounded' value={walletAddress} disabled />
          {addr !== undefined && (
            <a href={`https://solscan.io/account/${addr}`} target='_blank' className='text-sm text-right text-white'>View on Solscan</a>
          )}
        </div>
      </div>
      <div className='z-10 flex flex-col items-center gap-6 w-full'>
        <div className='flex gap-4'>
          ...
          {(wallet?.publicKey?.toBase58() == contractOwnerAddress || contractOwnerAddress == null) && addr === wallet?.publicKey?.toBase58() && (
        ...
        {currentTab === 'Coins Held' && (
          <div className='flex flex-col items-center gap-6 max-w-md w-full'>
                ...
                <div key={index} className='flex items-center gap-3 w-full'>
                  ...
                  <div className='flex flex-col w-full'>
                    <div className='flex justify-between'>
                      ...
                      <div className='text-sm text-white cursor-pointer'>[Refresh]</div>
                    </div>
                    <div className='flex justify-between'>
                      <p className='text-md text-[#97FF73]'>{item.lamports.toFixed(3)} SOL</p>
                      <Link href={`/token/${item.mintAddr}`} className='text-sm text-[#97FF73] cursor-pointer'>[View Coin]</Link>
                    </div>
                  </div>
                </div>
              )
            ...
            <div className='flex gap-6 justify-center'>
              <button type='button' className='text-md text-white'>{'[<<]'}</button>
              <button type='button' className='text-md text-white p-1'>1</button>
              <button type='button' className='text-md text-white'>{'[>>]'}</button>
            </div>
          </div>
        )}
                ...
                <div key={index} className='flex items-start gap-3 w-full'>
                  ...
                  <div className='flex flex-col'>
                    <div className='flex items-center gap-1'>
                      <p className='text-sm text-[#808080]'>Created by</p>
                      <Link href={`/profile/${item.walletAddr}`} className='flex items-center gap-1'>
                        <Image
                          ...
                          alt=""
                        />
                        <p className='text-sm text-[#97FF73]'>{item.username}</p>
                      </Link>
                    </div>
                    <p className='text-sm text-[#97FF73]'>market cap: {item.marketCap.toFixed(2)}K</p>
                    <p className='text-sm text-[#808080]'>replies: {item.replies}</p>
                    <p className='text-sm text-white'>{`${item.tokenName} (ticker: ${item.ticker}): ${item.desc}`}</p>
                  </div>
                </div>
              )
            })}
        ...
        {currentTab === 'Dashboard' && (wallet?.publicKey?.toBase58() == contractOwnerAddress || contractOwnerAddress == null) && addr === wallet?.publicKey?.toBase58() && (
          <div className='flex flex-col gap-4 w-full max-w-xl pb-4'>
            <div className="flex flex-col gap-2 w-full">
              <p className='text-2xl font-bold text-white'>Owner Address</p>
              <input value={ownerAddress} onChange={onChangeOwner} type="text" className={`w-full h-[69px] rounded-xl px-6 border border-white text-[#808080] bg-[#121212] text-base `} />
            </div>
            <div className="flex flex-col gap-2 w-full">
              <p className='text-2xl font-bold text-white'>Fee Recipent</p>
              <input value={feeRecipient} onChange={onChangeFeeRecipient} type="text" className={`w-full h-[69px] rounded-xl px-6 border border-white text-[#808080] bg-[#121212] text-base `} />
            </div>
            <div className="flex flex-col gap-2 w-full">
              <p className='text-2xl font-bold text-white'>Trading Fee (%)</p>
              <input value={tradingFee} onChange={onChangeTradingFee} type="number" className={`w-full h-[69px] rounded-xl px-6 border border-white text-[#808080] bg-[#121212] text-base `} />
            </div>
            <button type="button" className={`bg-white rounded-full w-full h-14 text-base `} onClick={handleDashboardSet}>Set</button>
          </div>
        )}
      </div>
            ...
            >
              <DialogPanel className="flex flex-col gap-10 p-10 w-full max-w-xl rounded-lg bg-[#1B1D28] border border-white backdrop-blur-2xl">
                ...
                <div className='flex flex-col gap-2'>
                  ...
                  <input type="text" className={`w-full h-[69px] bg-[#121212] pl-6 rounded-xl border border-white text-[#808080] text-base `} placeholder='Your Name' onChange={(e) => setUsername(e.target.value)} value={username} />
                </div>
                <div className="flex flex-col gap-2 w-full">
                  <p className='text-2xl font-bold text-white'>Bio</p>
                  <textarea className={`w-full h-[146px] rounded-xl px-6 py-4 border border-white text-[#808080] bg-[#121212] text-base resize-none `} placeholder='Bio' onChange={(e) => setBio(e.target.value)} value={bio}></textarea>
                </div>
